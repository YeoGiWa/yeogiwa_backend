...
@Tag(name = "⭐️ 즐겨찾기 API", description = "즐겨찾기 관a련 기능을 제공하는 API")
public class FavoriteController {

    private final FavoriteService favoriteService;

    @Operation(summary = "즐겨찾기 추가", description = "호스트와 이벤트 정보를 받아 즐겨찾기 추가")
    @ApiResponses({
    ...
    @PostMapping("/add")
    public FavoriteEntity addFavorite(@RequestParam UUID hostId, @RequestParam String eventId) {
        return favoriteService.addFavorite(hostId, eventId);
    }

    @Operation(summary = "즐겨찾기 삭제", description = "호스트와 이벤트 ID를 받아 해당 즐겨찾기 삭제")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "즐겨찾기 삭제 성공"),
            @ApiResponse(responseCode = "404", description = "즐겨찾기를 찾을 수 없음")
    })
    @DeleteMapping("/remove")
    public void removeFavorite(@RequestParam UUID hostId, @RequestParam String eventId) {
        favoriteService.removeFavorite(hostId, eventId);
    }

    @Operation(summary = "즐겨찾기 목록 조회", description = "호스트 ID를 받아 해당 호스트의 즐겨찾기 목록 조회")
    @ApiResponses({
