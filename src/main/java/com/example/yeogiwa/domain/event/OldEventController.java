...
//        GetEventResponse event = oldEventService.getEventById(id);
//
...
//        }
//
//        List<GetEventResponse> event = oldEventService.listEvents(numOfRows, pageNo, region, eventStartDate, eventEndDate, isValid);
//
//        return ResponseEntity.status(200).body(event);
//    }
...
//    ) {
//        List<GetEventResponse> event = oldEventService.listEventsNearby(numOfRows, pageNo, mapX, mapY, radius);
//
//        return ResponseEntity.status(200).body(event);
//    }
...
//        EventDto createdEventDto = oldEventService.createEvent(email, request);
//
...
//        EventDto updatedEventDto = oldEventService.updateEvent(id, request);
//
...
//        oldEventService.deleteEvent(id);
//    }
...
//        String email = userDto.getEmail();
//
//        List<EventDto> events = oldEventService.listEventsByAmbassador(email, isValid);
//
...
//                    @Content(array = @ArraySchema(schema = @Schema(implementation = FestivalCommonDto.class)))
//            }),
...
//    public ResponseEntity<List<FestivalCommonDto>> listFestivalsByKeyword(
//        @Parameter(description = "페이지에 보여줄 결과의 갯수 입니다. 기본 값은 10입니다.", example = "10") @RequestParam(name = "numOfRows", defaultValue = "10") int numOfRows,
//        @Parameter(description = "페이지 번호입니다. 1부터 시작합니다.", example = "1") @RequestParam("pageNo") int pageNo,
//        @Parameter(description = "검색할 축제의 키워드입니다.", example = "축제") @RequestParam(name = "keyword") String keyword
//
//    ) {
//        List<FestivalCommonDto> event = oldEventService.listFestivalsByKeyword(numOfRows, pageNo, keyword);
//
//        return ResponseEntity.status(200).body(event);
//    }
}